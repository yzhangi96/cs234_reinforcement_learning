...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", default=None, help="for problem1")
    parser.add_argument("--rlhf-directory", default=None, help="for problem2 rlhf")
    parser.add_argument("--dpo-directory", default=None, help="for problem3 dpo")
    parser.add_argument("--output", required=True, help="Path in which to save the output image")
    ...
    for seed in seeds:
        ...
        if dpo_directory is not None:
            ...
            all_results["SFT"].append(
                np.load(dpo_directory / f"Hopper-v4-sft-seed={seed}" / "scores.npy")
            )
        if rlhf_directory is not None:
            all_results["RLHF (original)"].append(
                np.load(rlhf_directory / f"Hopper-v4-rlhf-seed={seed}" / "original_scores.npy")
            )
            all_results["RLHF (learned)"].append(
                np.load(rlhf_directory / f"Hopper-v4-rlhf-seed={seed}" / "learned_scores.npy")
            )
